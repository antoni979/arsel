...
export async function buildTextPages(pdf, reportData) {
  ...
  const agruparPuntosConPlacaPorSala = () => {
      const puntosConPlaca = puntosInspeccionadosData.filter(pi => 
          pi.tiene_placa_caracteristicas === true && !pi.detalle_modificacion && pi.estado === 'existente'
      );
      if (puntosConPlaca.length === 0) return '';
      const grupos = {};
  ...
  pdf.setFontSize(FONT_SIZES.h2).setFont(undefined, 'bold'); pdf.text('3. RESULTADO DE LA VISITA', MARGIN, currentY); currentY += 8; pdf.setFontSize(FONT_SIZES.body).setFont(undefined, 'normal'); pdf.text('En rasgos generales los sistemas de almacenaje cumplen los parámetros establecidos en la memoria técnica de evaluación.', MARGIN, currentY, { maxWidth: DOC_WIDTH - (MARGIN * 2), lineHeightFactor: 1.5 }); currentY += 15; 
  ...
  const addSection = async (title, content) => {
    ...
    pdf.setFont(undefined, 'normal').setFontSize(FONT_SIZES.body).text(title, MARGIN, currentY);
    currentY += titleHeight;
    pdf.setFont(undefined, 'bold').text(content, MARGIN + 5, currentY, { lineHeightFactor: 1.5 });
    currentY += contentHeight;
  };

  if (textoVerdeAmbar) {
    pdf.setFont(undefined, 'bold').setFontSize(FONT_SIZES.body).text(textoVerdeAmbar, MARGIN, currentY, { lineHeightFactor: 1.5 });
    currentY += (textoVerdeAmbar.split('\n').length * 5) + 5;
  }
  
  // --- INICIO DE LA CORRECCIÓN DE FUENTE ---
  // Reseteamos el estilo a normal antes del siguiente párrafo
  pdf.setFont(undefined, 'normal').setFontSize(FONT_SIZES.body);
  // --- FIN DE LA CORRECCIÓN DE FUENTE ---
  ...
  await addSection('Alineaciones que disponen de sus fichas de características:', lineasConPlaca);
  ...
  pdf.setFont(undefined, 'bold').text('ARSEL INGENIERIA', MARGIN, currentY);
  currentY += 5;
  pdf.setFont(undefined, 'normal').text(`Valencia, ${fecha}`, MARGIN, currentY);
}