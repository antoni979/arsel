...
import { EyeIcon, TrashIcon, ArrowDownTrayIcon, CheckBadgeIcon, DocumentTextIcon, PaperAirplaneIcon, WrenchScrewdriverIcon } from '@heroicons/vue/24/outline';
...
// Lógica de estados adaptada a tu BBDD
...
const handleGeneratePdf = async (inspeccionId, reportType) => {
  isGeneratingPdf.value = true;
  await generatePdfReport(inspeccionId, reportType);
...
const handleDelete = async (inspeccionId) => {
  if (confirm('¿Estás seguro de que quieres borrar esta inspección? Esta acción es permanente y eliminará todos los datos y fotos asociados.')) {
    try {
      // 1. OBTENER TODAS LAS INCIDENCIAS DE LA INSPECCIÓN
      const { data: incidencias, error: getError } = await supabase
        .from('incidencias')
      ...
      // 2. CONSTRUIR LA LISTA DE ARCHIVOS A BORRAR
      const filesToDelete = [];
      if (incidencias && incidencias.length > 0) {
        incidencias.forEach(inc => {
          if (inc.url_foto_antes) {
            const filePath = inc.url_foto_antes.split('/incidencias/')[1];
            if (filePath) filesToDelete.push(filePath);
          }
          if (inc.url_foto_despues) {
            const filePath = inc.url_foto_despues.split('/incidencias/')[1];
            if (filePath) filesToDelete.push(filePath);
          }
        });
      }

      // 3. BORRAR LOS ARCHIVOS DE STORAGE (SI HAY ALGUNO)
      if (filesToDelete.length > 0) {
        ...
        if (storageError) {
          alert('Advertencia: No se pudieron borrar algunas fotos asociadas. Error: ' + storageError.message);
        }
      }

      // 4. FINALMENTE, BORRAR EL REGISTRO DE LA INSPECCIÓN DE LA BASE DE DATOS
          ...
          <li v-for="inspeccion in inspecciones" :key="inspeccion.id" class="p-4 grid grid-cols-1 lg:grid-cols-5 gap-6 items-center">
            ...
            <div class="lg:col-span-2 flex justify-end items-center flex-wrap gap-2">
              ...
              <div class="flex items-center gap-1 border-l pl-2 ml-2">
                <button @click="handleGeneratePdf(inspeccion.id, 'initial')" :disabled="isGeneratingPdf" class="p-2 text-slate-500 hover:text-blue-600 disabled:text-slate-300" title="Informe Inicial"><ArrowDownTrayIcon class="h-5 w-5" /></button>
                <button v-if="inspeccion.estado === 'cerrada'" @click="handleGeneratePdf(inspeccion.id, 'remediation')" :disabled="isGeneratingPdf" class="p-2 text-slate-500 hover:text-teal-600 disabled:text-slate-300" title="Informe Subsanación"><CheckBadgeIcon class="h-5 w-5"/></button>
